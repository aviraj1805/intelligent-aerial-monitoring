...
from results/*.json at build time, so the page cannot drift from the results.
    ...
    python scripts/render_demo.py                       # creates outputs/demo_annotated.mp4
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  a {{ color:var(--accent); }}
  video {{ width:100%; max-width:100%; border-radius:10px; border:1px solid var(--line); background:#000; margin-top:20px; }}
...
<main>
  ...
  <video src="demo_annotated.mp4" poster="poster.jpg" controls muted playsinline preload="metadata"></video>
  <p class="caption">Output of the pipeline on 24 drones flying in formation: boxes with track ID and
  confidence, recent path, and predicted path (orange). Source clip: "MagicLab – 24 Drone Flight" by
  Marco Tempest, <a href="https://creativecommons.org/licenses/by/3.0/" target="_blank" rel="noopener">CC BY 3.0</a>,
  via <a href="https://commons.wikimedia.org/wiki/File:MagicLab_-_24_Drone_Flight.webm" target="_blank" rel="noopener">Wikimedia Commons</a>,
  trimmed and annotated.</p>
...
def main():
    ...
    video = ROOT / "outputs" / "demo_annotated.mp4"
    poster = ROOT / "outputs" / "poster.jpg"
    if not video.exists():
        raise SystemExit("Run scripts/render_demo.py first (outputs/demo_annotated.mp4 is missing).")
    ...
    def write(stage: Path):
        ...
        shutil.copy2(video, stage / "demo_annotated.mp4")
        if poster.exists():
            shutil.copy2(poster, stage / "poster.jpg")
